...
async def _jr_campaign_loop(
    ...
    """
    HIGH-SPEED Join Request DM campaign using queue-worker architecture.
    Sends DMs to pending join requesters concurrently WITHOUT accepting/dismissing them.
    """
    client = _clients.get(user_id)
    if not client or not await client.is_user_authorized():
        await done_cb(user_id, "Not authorised. Please add your account first.")
    ...
    try:
        ...
        # Fetch up to how_many requesters, paginating 100 at a time.
        # Telegram caps GetChatInviteImportersRequest results per call (often ~100)
        # regardless of the limit parameter. The ONLY reliable signal that there
        # are no more pages is an empty importers list — receiving fewer than
        # batch_limit does NOT mean we're at the end, so we never break early.
        fetched = []
        offset_date = 0
        offset_user = InputUserEmpty()
        ...
        while len(fetched) < how_many:
            ...
            if not result.importers:
                break  # empty page = truly exhausted
            fetched.extend(result.importers)
            if len(fetched) >= how_many:
                break  # we have enough
            # Advance the pagination cursor to the last returned item
        ...
        # 15 concurrent workers for JR DM (slightly lower than broadcast to be safe)
        _JR_CONCURRENCY = 15
        _UPDATE_EVERY = 10
        ...
        for imp in importers:
            queue.put_nowait(imp)

        async def _worker():
            nonlocal sent_count, failed_count, privacy_blocked
            while True:
                try:
                    importer = queue.get_nowait()
                except asyncio.QueueEmpty:
                    break
                ...
                try:
                    ...
                    async with lock:
                        sent_count += 1
                        done_so_far = sent_count + failed_count + privacy_blocked
                    ...
                    # User has privacy settings preventing DMs
                    async with lock:
                        privacy_blocked += 1
        ...
        # Launch all workers
        workers = [asyncio.create_task(_worker()) for _ in range(_JR_CONCURRENCY)]
        await asyncio.gather(*workers)
