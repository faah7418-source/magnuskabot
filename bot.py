...
async def _create_order_and_send(update, ctx, plan_key: str, mode: str):
    ...
    # Try to send with QR photo — prefer DB-stored file_id, fall back to built-in base64
    sent = None
    custom_qr_file_id = await db.get_setting("qr_image_file_id", "")
    try:
        photo_src = custom_qr_file_id if custom_qr_file_id else get_image_bytes("qr")
...
async def handle_jr_channel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    wait_msg = await update.message.reply_text(
        f"🔍 *Analysing channel…*\n\n"
        f"📣 Channel: `{label}`\n"
        f"_Fetching pending join requests, please wait…_",
        parse_mode="Markdown",
    )
    ...
    try:
        ...
        await wait_msg.edit_text(
            ...
            f"Error: `{ex}`\n\n"
            "Please check:\n"
            "• Channel link/username is correct\n"
            "• Your linked account is an *admin* of that channel\n"
            "• Join requests are *enabled* in channel settings\n"
            "• For private channels, use the full invite link\n"
            f"  e.g. `https://t.me/+xxxxxxxx`",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔄 Try Again", callback_data="cb_jr_dm")],
                [InlineKeyboardButton("🔙 Back to Menu", callback_data="cb_back")],
            ]),
        )
        return ConversationHandler.END
    ...
    if total == 0:
        await wait_msg.edit_text(
            ...
            "_Enable join requests in channel settings and try again._",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔄 Try Again", callback_data="cb_jr_dm")],
                [InlineKeyboardButton("🔙 Back to Menu", callback_data="cb_back")],
    ...
    # Show nice analysis result with send-all button
    await wait_msg.edit_text(
        f"✅ *Channel Analysed Successfully!*\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📣 Channel: `{label}`\n"
        f"👥 Total Pending Requests: *{total:,}*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📨 *How many people do you want to DM?*\n\n"
        f"• Send a number *(e.g. `100`, `{total}`)* \n"
        f"• Max allowed: *{total:,}*\n\n"
        f"⚠️ _Join requests will NOT be accepted — only DMs will be sent._",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(f"📨 Send to ALL {total:,} requests", callback_data="jr_send_all")],
            [InlineKeyboardButton("🔙 Cancel", callback_data="cb_back")],
        ]),
    )
...
async def handle_jr_count(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"⚠️ *Invalid Number*\n\n"
            f"Please send a valid number between 1 and {total:,}.\n"
            f"Example: `{min(100, total)}`",
            parse_mode="Markdown",
        )
        return JR_COUNT_INPUT
    ...
    if count > total:
        count = total
        await update.message.reply_text(
            f"ℹ️ Count adjusted to *{total:,}* (maximum available).",
            parse_mode="Markdown",
        )

    await _jr_confirm_count(update.message.reply_text, ctx, count, label)
    return JR_MSG_COLLECT


async def _jr_confirm_count(reply_fn, ctx, count: int, label: str):
    """Shared — sets count in session and prompts for message."""
    ...
    await reply_fn(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📣 Channel: `{label}`\n"
        f"📨 DMs to send: *{count:,}* people\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📝 *Now send the message you want to DM them.*\n\n"
        f"💡 Supported:\n"
        f"• Text & links\n"
        f"• Images / photos\n"
        f"• Files / documents\n"
        f"• Multiple messages (each sent separately)\n\n"
...
async def cb_jr_send_all(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """User tapped Send to ALL — skip typing a number."""
    q = update.callback_query
    await q.answer()
    total = ctx.user_data.get("jr_total", 0)
    label = ctx.user_data.get("jr_label", "")
    if not total:
        await q.message.reply_text(
            "⚠️ Session expired. Please tap *Join Request DM* again.",
            parse_mode="Markdown",
            reply_markup=main_menu_kb(),
        )
        return ConversationHandler.END

    await _jr_confirm_count(q.message.reply_text, ctx, total, label)
    return JR_MSG_COLLECT
