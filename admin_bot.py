...
def edit_menu_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎬 Set Tutorial Video", callback_data="ae_tutorial_video")],
        [InlineKeyboardButton("🖼️ Change QR Image", callback_data="ae_qr_image")],
...
# ── Tutorial Video ────────────────────────────────────────────────────────────
async def cb_edit_tutorial_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    if not await admin_only(update):
        return ConversationHandler.END

    current_file_id = await db.get_setting("tutorial_video_file_id", None)
    status = f"✅ Video is SET (`{current_file_id[:20]}...`)" if current_file_id else "❌ No video set yet"

    await q.message.reply_text(
        f"🎬 *TUTORIAL VIDEO SETTINGS*\n"
        f"{'─' * 22}\n\n"
        f"📹 Current Status: {status}\n\n"
        f"Send a *video* now to set/replace the tutorial video.\n"
        f"Users will see this when they tap *📖 How to Use*.\n\n"
        f"_You can replace it anytime by sending a new video._",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🗑 Remove Video", callback_data="ae_tutorial_video_remove")],
            [InlineKeyboardButton("🔙 Cancel", callback_data="a_edit")],
        ]),
    )
    return EDIT_TUTORIAL_VIDEO_INPUT


async def handle_tutorial_video_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update):
        return ConversationHandler.END
    ...
    await msg.reply_text(
        "✅ *Tutorial video saved!*\n\n"
        "Users will now see this video when they tap *📖 How to Use*.",
        parse_mode="Markdown",
        reply_markup=edit_menu_kb(),
    )
    return ConversationHandler.END
...
async def cb_tutorial_video_remove(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    if not await admin_only(update):
        return
    await db.set_setting("tutorial_video_file_id", "")
    await q.message.reply_text(
        "🗑 *Tutorial video removed.*\n\n"
        "Users will now see the default text tutorial.",
        parse_mode="Markdown",
        reply_markup=edit_menu_kb(),
    )
...
# ── QR Image ──────────────────────────────────────────────────────────────────
async def cb_edit_qr_image(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    if not await admin_only(update):
        return ConversationHandler.END

    has_custom = await db.get_setting("qr_image_file_id", None)
    status = f"✅ Custom QR is SET" if has_custom else "📦 Using default built-in QR"

    await q.message.reply_text(
        f"🖼️ *CHANGE PAYMENT QR IMAGE*\n"
        f"{'─' * 22}\n\n"
        f"📌 Current Status: {status}\n\n"
        f"Send a *photo* now to set/replace the QR image.\n"
        f"This image is shown to users when they buy a premium plan.\n\n"
        f"_You can replace it anytime by sending a new photo._",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🗑 Remove Custom QR (use default)", callback_data="ae_qr_image_remove")],
            [InlineKeyboardButton("🔙 Cancel", callback_data="a_edit")],
        ]),
    )
    return EDIT_QR_IMAGE_INPUT


async def handle_qr_image_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update):
        return ConversationHandler.END
    ...
    if not msg.photo:
        await msg.reply_text(
            "⚠️ Please send a *photo* (image file).",
            parse_mode="Markdown",
        )
        return EDIT_QR_IMAGE_INPUT

    file_id = msg.photo[-1].file_id
    await db.set_setting("qr_image_file_id", file_id)
    await msg.reply_text(
        "✅ *QR Image updated!*\n\n"
        "Users will now see your new QR image when buying a premium plan. 🚀",
        parse_mode="Markdown",
        reply_markup=edit_menu_kb(),
    )
    return ConversationHandler.END
...
async def cb_qr_image_remove(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    if not await admin_only(update):
        return
    await db.set_setting("qr_image_file_id", "")
    await q.message.reply_text(
        "🗑 *Custom QR removed.*\n\n"
        "The default built-in QR image will be used.",
        parse_mode="Markdown",
        reply_markup=edit_menu_kb(),
    )
...
async def handle_broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if total == 0:
        await msg.reply_text(
            "⚠️ *No users found for this filter.*",
            parse_mode="Markdown",
            reply_markup=admin_menu_kb(),
        )
        return ConversationHandler.END
    ...
    # ── High-speed queue-worker architecture ──────────────────────────────────
    # 30 concurrent workers — saturates Telegram's bot API limit safely
    _BC_CONCURRENCY = 30
    _UPDATE_EVERY = 25   # update progress counter every N sends
    ...
    async def _worker():
        nonlocal sent, blocked, failed, flood_waits
        while True:
            try:
                u = queue.get_nowait()
            ...
            try:
                ...
                async with lock:
                    sent += 1
                    done_so_far = sent + blocked + failed
                    if done_so_far % _UPDATE_EVERY == 0 or done_so_far == total:
                        elapsed = max(time.monotonic() - start_time, 0.1)
                        speed = sent / elapsed
                        pct = round(100 * done_so_far / total)
                        filled = round(pct / 5)
                ...
                err = str(e).lower()
                if any(k in err for k in ("blocked", "chat not found", "deactivated", "bot was blocked", "forbidden", "kicked")):
                    async with lock:
                        blocked += 1
    ...
    workers = [asyncio.create_task(_worker()) for _ in range(_BC_CONCURRENCY)]
    await asyncio.gather(*workers)

    elapsed = max(time.monotonic() - start_time, 0.1)
    speed_avg = sent / elapsed
    mins = int(elapsed // 60)
    secs = int(elapsed % 60)
    ...
    await status_msg.edit_text(
        ...
        f"🚀 Avg Speed: *{speed_avg:.1f}* msg/s",
        parse_mode="Markdown",
    )
...
def build_app():
    ...
    tutorial_video_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_edit_tutorial_video, pattern="^ae_tutorial_video$")],
        states={EDIT_TUTORIAL_VIDEO_INPUT: [MessageHandler(filters.VIDEO & ~filters.COMMAND, handle_tutorial_video_input)]},
        fallbacks=[CommandHandler("cancel", cancel_conv)],
        allow_reentry=True, per_message=False,
    )
    ...
    app.add_handler(tutorial_video_conv)
    app.add_handler(CallbackQueryHandler(cb_tutorial_video_remove, pattern="^ae_tutorial_video_remove$"))

    qr_image_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_edit_qr_image, pattern="^ae_qr_image$")],
        states={EDIT_QR_IMAGE_INPUT: [MessageHandler(filters.PHOTO & ~filters.COMMAND, handle_qr_image_input)]},
        fallbacks=[CommandHandler("cancel", cancel_conv)],
        allow_reentry=True, per_message=False,
    )
    app.add_handler(qr_image_conv)
    app.add_handler(CallbackQueryHandler(cb_qr_image_remove, pattern="^ae_qr_image_remove$"))
